  ...
  return (
    <div className="container py-4">
      ...
      <div className="row mb-4">
        {[
          { label: 'Total Enquiries', count: contacts.length, color: 'info' },
          { label: 'Admissions Done', count: admissions.length, color: 'success' },
          { label: "Today's Enquiries", count: todayContacts.length, color: 'warning' },
          { label: 'This Month', count: monthlyContacts.length, color: 'primary' },
        ].map((card, i) => (
          <div className="col-md-3 mb-3" key={i}>
            <div className={`card bg-${card.color} text-white h-100`}>
              <div className="card-body">
                <h5 className="card-title">{card.label}</h5>
                <p className="display-6">{card.count}</p>
              </div>
            </div>
          </div>
        ))}
      ...
      <div className="card mb-4">
        <div className="card-body row g-3 align-items-end">
          <div className="col-md-2">
            ...
            <input type="number" className="form-control" min="1" max="12" value={monthFilter} onChange={(e) => setMonthFilter(e.target.value)} />
          </div>
          <div className="col-md-2">
          ...
          <div className="col-md-2">
            <button className="btn btn-secondary w-100" onClick={() => { setMonthFilter(''); setDateFilter(''); }}>Clear Filters</button>
          </div>
          <div className="col-md-3">
            <button className="btn btn-danger w-100" onClick={clearAll}>Clear All Entries</button>
          </div>
          <div className="col-md-3">
            <button className="btn btn-danger w-100" onClick={clearByMonth}>Clear By Month</button>
          </div>
        </div>
      </div>
              ...
              <tr key={c._id}>
                ...
                <td>
                  <button
                    ...
                    onClick={(e) => { e.stopPropagation(); toggleAdmission(c._id, c.admission); }}
                  >
                    {c.admission ? '✅ Done' : '❌ Pending'}
                  </button>
                </td>
                <td>
                  <button className="btn btn-sm btn-primary me-1" onClick={() => setEditingContact(c)}>Edit</button>
                  <button className="btn btn-sm btn-danger" onClick={(e) => { e.stopPropagation(); handleDelete(c._id); }}>Delete</button>
                </td>
              </tr>
            ))}
          </tbody>
              ...
              {['name', 'email', 'phone', 'subject', 'message'].map((field) => (
                <div className="col-md-6" key={field}>
                  ...
                  <input
                    ...
                    onChange={(e) => setEditingContact({ ...editingContact, [field]: e.target.value })}
                    required
                  />
                </div>
              ))}
