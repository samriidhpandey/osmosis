...
const RSCITRoadmap = () => {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "linear-gradient(to bottom, #f8d7da, white)", padding: "3rem 1rem" }}>
      <h1 style={{ fontSize: "2.5rem", fontWeight: "bold", textAlign: "center", color: "#8B0000", marginBottom: "3rem" }}>
        RS-CIT Course Roadmap
      </h1>

      <div style={{ maxWidth: "960px", margin: "0 auto", borderLeft: "4px solid #8B0000", paddingLeft: "1.5rem", marginBottom: "4rem" }}>
        {rscitRoadmap.map((step, index) => (
          <motion.div
            ...
            style={{ backgroundColor: "white", boxShadow: "0 2px 8px rgba(0,0,0,0.1)", borderRadius: "1rem", padding: "1.5rem", marginBottom: "1.5rem", transition: "transform 0.3s" }}
          >
            <h3 style={{ fontSize: "1.25rem", fontWeight: "600", color: "#8B0000" }}>
              Step {index + 1}: {step.title}
            </h3>
            <p style={{ color: "#555", marginTop: "0.5rem", fontSize: "0.875rem" }}>{step.desc}</p>
          </motion.div>
        ))}
      </div>

      <div style={{ maxWidth: "1140px", margin: "0 auto" }}>
        <h2 style={{ fontSize: "2rem", fontWeight: "bold", color: "#8B0000", marginBottom: "1.5rem", textAlign: "center" }}>Frequently Asked Questions</h2>
        <div className="row" style={{ display: "flex", gap: "1rem", flexWrap: "wrap" }}>
          <div className="col-12 col-md-6" style={{ flex: 1 }}>
            {faqsLeft.map((faq, index) => (
              <div
                key={index}
                style={{ border: "1px solid #f5c6cb", borderRadius: "1rem", backgroundColor: "white", boxShadow: "0 1px 4px rgba(0,0,0,0.1)", padding: "1rem", marginBottom: "1rem" }}
              >
                <button
                  style={{ width: "100%", textAlign: "left", fontWeight: "600", color: "#8B0000", background: "none", border: "none", display: "flex", justifyContent: "space-between", alignItems: "center" }}
                ...
                {openIndexLeft === index && (
                  <motion.p
                    ...
                    style={{ marginTop: "0.5rem", color: "#555", fontSize: "0.875rem" }}
                  >
                    {faq.answer}
                  </motion.p>
                )}
