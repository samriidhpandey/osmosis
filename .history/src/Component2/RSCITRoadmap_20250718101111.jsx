...
const RSCITRoadmap = () => {
  ...
  const bg = darkMode ? "#121212" : "linear-gradient(to bottom, #f8d7da, white)";
  const cardBg = darkMode ? "#1e1e1e" : "white";
  const textColor = darkMode ? "#f1f1f1" : "#8B0000";
  const paraColor = darkMode ? "#aaa" : "#555";
  const borderColor = darkMode ? "#333" : "#f5c6cb";
  const shadow = darkMode ? "0 2px 8px rgba(255,255,255,0.05)" : "0 2px 8px rgba(0,0,0,0.1)";
      ...
      <div style={{ maxWidth: "960px", margin: "0 auto", borderLeft: `4px solid ${textColor}`, paddingLeft: "1.5rem", marginBottom: "4rem" }}>
        {rscitRoadmap.map((step, index) => (
          <motion.div
            ...
            style={{ backgroundColor: cardBg, boxShadow: shadow, borderRadius: "1rem", padding: "1.5rem", marginBottom: "1.5rem", transition: "transform 0.3s" }}
          >
